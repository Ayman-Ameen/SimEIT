function mdl= mk_fmdl_from_nodes( vtx, elec_nodes,perimeter_nodes, z_contact, name)
...
for i= 1:prod(size(elec_nodes)) 
   ...
   boundary_nodes = [boundary_nodes;elec_nodes{i}];
end

for i= 1:prod(size(perimeter_nodes)) 
   vtx            = [vtx; perimeter_nodes{i}];
   boundary_nodes = [boundary_nodes;perimeter_nodes{i}];
end

% scatter(vtx(mdl.boundary(:,1),2),vtx(mdl.boundary(:,1),1))
% patch('Faces',mdl.boundary  ,'Vertices',vtx  ,'EdgeColor','green','FaceColor','red','LineWidth',0.2)
% hold on


% Sort and remove duplicates
vtx = unique(vtx, 'rows');
% remove the last point which close to each other.
vtx = vtx([1:end-2,end],:);
...
% mdl.boundary = find_boundary_circle(boundary_nodes,mdl.nodes,mdl.elems);
...
%%%%%%%%%%%%%%%%%%%%%%%%%
% for counter = 1:length(elec_nodes)
% hold on
% scatter(elec_nodes{counter}(:,1)',elec_nodes{counter}(:,2)')
% pause(1)
% hold on
% end
% for counter = 1:length(perimeter_nodes)
% scatter(perimeter_nodes{counter}(:,1)',perimeter_nodes{counter}(:,2)')
% pause(1)
% hold on
% end   
